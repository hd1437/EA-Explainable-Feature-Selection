...
from FS.functionHO import Fun, Acc_test
...
def jfs(xtrain, ytrain, opts):
    ...
    curve_test = np.zeros([1, max_iter], dtype='float')
    t     = 0
    X_best_set.append(np.append(t,Xgb[0]))
    curve[0,t] = fitG.copy()
    ...
    while t < max_iter:
        ...
        # Store result
        curve[0,t] = fitG.copy()
        curve_test[0,t] = Acc_test(Xgb[0], opts)
        print("Generation:", t + 1)
        print("Best (GA):", curve[0,t])
        t += 1
    ...
    # Create dictionary
    ga_data = {'sf': sel_index, 'c': curve, 'nf': num_feat, 'ct':curve_test}
    import csv
    import os
    folder_path = r'../record/'+opts['dataset']+r'/figure/ga'
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    csv_file = r'../record/'+opts['dataset']+r'/figure/ga/selected_features_comparison.csv'
    # 使用 CSV 模块写入数据
    with open(csv_file, mode="a+", newline="") as file:
        writer = csv.writer(file)
        writer.writerows(X_best_set)
    return ga_data